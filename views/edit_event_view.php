		...
		<div class="container py-5">
			<div class="row justify-content-center">
				<div class="col-12 col-sm-10 col-md-8 col-lg-6">
					<h1 class="mb-4 text-center"><?= __t('edit_event') ?></h1>
					<form action="<?= app_url('page/editEvent?id=' . urlencode((string)$eventData['id'])) ?>" method="post">
						<div class="mb-3">
						...
						<div class="mb-3">
							...
							<textarea name="description" id="description" class="form-control"><?= htmlspecialchars($eventData['description']) ?></textarea>
						</div>
						<div class="mb-3">
							...
							value="<?= str_replace(' ', 'T', htmlspecialchars($eventData['event_date'])) ?>" required>
						</div>
						<div class="mb-3">
							...
							<select name="event_type_id" id="event_type_id" class="form-select" required>
								...
								<option value="<?= htmlspecialchars($type['id']) ?>" <?= $type['id'] == $eventData['event_type_id'] ? 'selected' : '' ?>>
...
<div class="mb-3">
  ...
  <div class="d-flex flex-wrap gap-2">
    <?php foreach ($musicGenres as $g): 
      $isChecked = in_array($g['id'], $selectedGenres, true);
    ?>
      <input type="checkbox"
             class="btn-check"
             name="genres[]"
             ...
             autocomplete="off"
             <?= $isChecked ? 'checked' : '' ?>>
      <label class="btn <?= $isChecked ? 'btn-primary' : 'btn-outline-secondary' ?>"
             for="genre-<?= $g['id'] ?>">
    ...
    <?php endforeach; ?>
						...
						<div class="mb-3">
							...
							<select name="venue_id" id="venue_id" class="form-select" onchange="toggleNewVenueFields()" required>
								...
								<option value="<?= htmlspecialchars($venue['id']) ?>" <?= $venue['id'] == $eventData['venue_id'] ? 'selected' : '' ?>>
